...
<?php if (!isset($new_page)){$new_page = false;}?>
  ...
  <p>Page Name: <input type="text" name="menu_name" value="<?php echo $sel_page['menu_name'];?>" id="menu_name" /></p>
	<p>Position:<select name="position">
		<?php
		if(!$new_page)//edit page, only show positions entered
		{
			$page_set = get_pages_for_subject($sel_page['subj_id']);
			$page_count = mysql_num_rows($page_set);
			
		}	
		else //new page, show positions + 1
		{
	...
	<p>Content:<br>
	<textarea name="content" rows="20" cols="80"><?php echo $sel_page['content'];?></textarea></p>		
